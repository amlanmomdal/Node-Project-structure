...
const register = async (req, res) => {
    ...
    const formattedDate = changeDateFormatTo(todayDate);
    let reffer_id = makeid() + formattedDate;
    let UserData = {
        ...req.body,
        refer_code: reffer_id,
...
const login = async (req, res) => {
    ...
    return User.findOne({ email: req.body.email }, async (err, admin) => {
        if (err) {
            res.status(200).json({
                status: false,
